...
```
# Figure S2: Abiotic parameters
```{r}

# Step 1: Calculate mean + standard deviation (and standard error) per group
avg_values <- dfab %>% 
  group_by(SAMPLE_strata, sample_depth_m, parameter, Location) %>%
  summarise(
    average = mean(as.numeric(value), na.rm = TRUE),
    sd = sd(as.numeric(value), na.rm = TRUE),
    n = n(),
    se = sd / sqrt(n) # standard error
  )

# Step 2: Plot with horizontal error bars
ggplot(avg_values, aes(x = average, y = sample_depth_m, color = parameter)) +
  geom_jitter() +
  geom_point(size = 2) +
  geom_errorbarh(aes(xmin = average - se, xmax = average + se), height = 0.5) +
  scale_y_reverse(limits = c(100, 0), breaks = seq(0, 100, by = 10)) +
  facet_grid(Location ~ parameter, scales = "free_x") + 
  labs(
    y = "Depth (m)",
    x = "Value",
...
syndiseason <- Syndiniales %>%
  mutate(Season = ifelse(SAMPLE_date %in% c("2019-06-01", "2017-06-19", "2019-06-08", "2019-06-07", "2019-06-13", "2019-06-05", "2019-06-12", "2019-06-03", "2019-06-02", "2019-06-06",
                                            "2019-06-11", "2019-06-04", "2019-06-09"),           
...
# GLM analysis, Table S3
- "wide_merged" containd the 18S data and the corresponding abiotic data
